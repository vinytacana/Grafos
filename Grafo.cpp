...
void heapifyDown(Heap heap[], int posicao[], int tamanhoHeap, int pai) //correta
{
	...
	//busca encontrar qual dos filhos tem a menor distancia, se encontrar, altera o indice da posiï¿½ï¿½o com a menor distancia.
    ...
    if (minimo != pai) //se o pai nï¿½o era o menor valor, entï¿½o realiza a troca e chama a funï¿½ï¿½o para continuar a rebaixar o novo valor pai.
	{
...
void adicionaNaHeap(Heap heap[], int posicao[], int *tamanhoDaHeap, Heap element)//correta
{
	...
	else
	{
	    ...
	    if(*tamanhoDaHeap > 1)//so comeï¿½a se haver pelo menos 2 elementos na heap
...
void dijkstraMatrizSemHeap(GrafoMatrizAdj *grafo, int verticeInicial)
{
    ...
    for (count = 0; count < (grafo->qtdVertices - 1); count++)//garantir que se repita para todos os vertices.
	{
        ...
        for (v = 0; v < grafo->qtdVertices; v++)
			...
			// e o peso total do caminho do verticeInicial para V atraves de U ï¿½ o menor que o valor atual de distancia[V] 
...
void criarGrafo(GrafoMatrizAdj **grafo, int qtdVertices)
{
    ...
    tempGrafo->hasNegativeWeights = 0;
...
void lerArquivoDeGrafoMatriz(GrafoMatrizAdj **grafo, const char *nomeArquivo)
{
    //DESTINADO A MATRIZ DE ADJACENCIA
	//COMENTARIO PARA A EXPLICACAO DESSA FUNCAO----
    ...
    sscanf(buffer, "%d", &qtdVertices);//primeiro elemento a ser lido eh a quantidade de vertices;
    ...
    *grafo = NULL;//inicializa o grafo com NULL
    ...
    {	
        int itensLidos = sscanf(buffer, "%d %d %f", &i, &j, &peso);
        if (itensLidos < 2) continue;
        if (itensLidos == 2) peso = 1.0; // unweighted
        ...
        // ajustando os Ã­ndices pois no arquivo costuma ser 1-based, e na matriz 0-based
        i--; j--;
		//grafo nao direcionado
    	(*grafo)->grafo[i][j] = peso;
    	(*grafo)->grafo[j][i] = peso;
        if (peso < 0) (*grafo)->hasNegativeWeights = 1;
		linha++;
	}
...
void bfsMatriz(GrafoMatrizAdj *grafo, int verticeInicial)
{
	...
	//	visitando todos os vertices adjacentes e colocando os nao visitados em uma fila. Apï¿½s enfileirar todos os vertices adjacentes 
...
int grauDoVertice(GrafoMatrizAdj *grafo, int vertice)
{
	...
	//MODIFICADO:
	//	Agora essa funï¿½ï¿½o lida com pesos entre os vertices
...
int grauMinimo(GrafoMatrizAdj *grafo)
{
	//DESTINADO A MATRIZ DE ADJACENCIA
	//COMENTARIO PARA A EXPLICACAO DESSA FUNCAO-----
	//	Essa funcao tem o objetivo de encontrar o grau minimo de um grafo, o grau minimo ï¿½ o menor grau entre todos os outros
...
int grauMaximo(GrafoMatrizAdj *grafo)
{
	//DESTINADO A MATRIZ DE ADJACENCIA
	//COMENTARIO PARA A EXPLICACAO DESSA FUNCAO-----
	//	Essa funcao tem o objetivo de encontrar o grau maximo de um grafo, o grau maximo ï¿½ o maior grau entre todos os outros
...
int contarArestasMatriz(GrafoMatrizAdj *grafo)
{
	...
	//DATA: 05/07/2024. HORï¿½RIO: 21:28. AUTOR: MIKAEL PEREIRA GOMES.
...
void informacoesGrafoMatrizToArq(GrafoMatrizAdj *grafo)
{
	...
	fprintf(arq,"DIAMETRO DO GRAFO %d\n", diametro);
...
void lerArquivoDeGrafoLista(GrafoListaAdj **grafo, const char *nomeArquivo)
{
    FILE *arquivo = fopen(nomeArquivo,"r");
    if(arquivo == NULL)
    {
    ...
    sscanf(buffer, "%d", &qtdVertices);

    *grafo = NULL;

    criarGrafoLista(grafo, qtdVertices);
    if(*grafo == NULL)
    {
